...
class PcapFileHeader : public Stackable
{
  public:
    ...
    Utility::ObservableProperty<uint32_t> MagicNumber;

    /// @brief メジャーバージョンを設定/取得する．
    Utility::ObservableProperty<uint16_t> VersionMajor;

    /// @brief マイナーバージョンを設定/取得する．
    Utility::ObservableProperty<uint16_t> VersionMinor;

    /// @brief タイムゾーンを設定/取得する．
    Utility::ObservableProperty<uint32_t> ThisZone;

    /// @brief タイムスタンプの精度を設定/取得する．
    Utility::ObservableProperty<uint32_t> SigFigs;

    /// @brief キャプチャファイルの最大長を設定/取得する．
    Utility::ObservableProperty<uint32_t> SnapLen;

    /// @brief ネットワークを設定/取得する．
    Utility::ObservableProperty<uint32_t> LinkType;
